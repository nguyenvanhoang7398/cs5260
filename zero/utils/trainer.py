...
class Trainer(object):
    def __init__(self, args, model, train_source_dataloader, train_target_dataloader,
    ...
    def train(self):
        ...
        with tqdm(total=self.num_train_steps, disable=self.args.local_rank not in (-1, 0)) as pbar:
            while True:
                ...
                train_results, train_report = evaluate(self.args, model, "train", None,
                                                       return_report=True)
                dev_results, dev_report = evaluate(self.args, model, "dev", None,
                                                   return_report=True)
                test_results, test_report = evaluate(self.args, model, "test", None,
                                                     return_report=True)
