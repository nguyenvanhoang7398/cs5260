...
def load_domain_features(args, src_domain=None, trg_domain=None, data_dir=None):
    ...
    return domain_indices, word_embedding
...
def load_and_cache_examples(args, fold, inter_domain_entities, random_sampling=True):
    ...
    # Fif os.path.exists(cache_file):
    domain_tag = "{}_{}".format(*sorted([args.dev_domain, args.test_domain]))
    inter_domain_entities = load_text_as_list(
        os.path.join(args.concept_graph_dir, domain_tag, "domain_entities.txt"))
    ...
    def collate_fn(batch):
        ...
        ret = dict(
            ...
            concept_ratios=create_padded_sequence("concept_ratios", 0)
        )
