...
@click.option("--dev-domain", default="music")
...
def run(common_args, **task_args):
    ...
    domain_label_indices, domain_features = \
    ...
    train_source_dataloader, _, _, processor = load_and_cache_examples(args, "train")
    train_target_dataloader, _, _, _ = load_and_cache_examples(args, "test")
    results = {}
    ...
    if args.do_train:
        ...
        trainer = Trainer(args, model=dozen,
    ...
    if args.do_eval:
        ...
        results.update({f"train_{k}": v for k, v in evaluate(args, zero, "train",
                                                             train_output_file).items()})
        results.update({f"dev_{k}": v for k, v in evaluate(args, zero, "dev",
                                                           dev_output_file).items()})
        results.update({f"test_{k}": v for k, v in evaluate(args, zero, "test",
                                                            test_output_file).items()})
